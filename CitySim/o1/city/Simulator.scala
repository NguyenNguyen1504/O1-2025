...
class Simulator:
  ...
  def findDemographic(demographic: Demographic): Vector[GridPos] =
    this.currentMap.allAddresses.filter( this.currentMap.matches(_,demographic))

     // TODO: implementation missing


  /** Returns all the locations in the city being simulated. */
  def allAddresses: Vector[GridPos] = this.cityMap.allAddresses
  ...
  def dissatisfiedResidents: Vector[GridPos] =
    def allResidents = this.currentMap.allAddresses.filter( this.currentMap.elementAt(_) != Vacant )
    def similarNeighborsRatio(address: GridPos) =
      val allOccupiedNeighbors = this.currentMap.neighbors(address, true).filter( _ != Vacant )
      val residentType = this.currentMap.elementAt(address)
      def similarDemo(neighbor: Demographic, thisResident: Demographic) =
        (neighbor, thisResident) match
          case (n: Occupied, r: Occupied) => n.label == r.label
          case _ => false

      val sameDemoNeighbors = allOccupiedNeighbors.count( similarDemo(_, residentType) )
      if allOccupiedNeighbors.isEmpty then
        100.0
      else
       sameDemoNeighbors.toDouble / allOccupiedNeighbors.size.toDouble * 100.0

    allResidents.filter( similarNeighborsRatio(_) < this.similarityDesired )
// Lọc ra positions có ngừoi ở: allResidents -> Vector[GridPos]
// Hàm tính tỉ lệ cùng demo
// 1. Lọc ra hàng xóm (Loại ô trống) -> Vector[Demographic]
// 2. Tạo hàm kiểm tra cùng demo
// 3. Kiểm tra hàng xóm cùng demo ko?
  ...
  /** Advances the most recently launched simulation by one step, moving all dissatisfied
    ...
    * X’s neighborhood. X will only move the next time `moveResidents` is called, if X
  ...
  def moveResidents(): Unit =
    ...
    val unsatisfiedAddresses = Random.shuffle(this.dissatisfiedResidents)
    if vacantAddresses.nonEmpty then
      def updateAddress(old: GridPos) =
        val randomIndex = Random.nextInt(vacantAddresses.size)
        val randomAddress = vacantAddresses(randomIndex)
        this.currentMap.update(randomAddress, this.currentMap.elementAt(old))
        this.currentMap.update(old, Vacant)
        vacantAddresses(randomIndex) = old
      unsatisfiedAddresses.foreach( updateAddress(_) )
