...
class Area(var name: String, var description: String):
  ...
  def fullDescription: String =
    ...
    val itemList =
      if this.itemsContainer.isEmpty then
        ""
      else
        "\n\nYou see here: " + this.itemsContainer.keys.mkString(" ")
    this.description + itemList + exitList

  def addItem(item: Item) =
    this.itemsContainer(item.name) = item

  def removeItem(itemName: String): Option[Item] =
    val existence = this.itemsContainer.get(itemName)
