...
class Category(val name: String, val unit: String):
  ...
  def addExperience(newExperience: Experience) =
    ...
    // Note to students: the above is not the neatest way to implement this functionality, but it’s
    // the one we’ll study first. We’ll explore alternative implementations later in O1’s ebook.
