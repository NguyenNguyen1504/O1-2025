...
/** Each instance of the class `Moment` represents a “moment” on a timescale,
  ...
  *
  * @param time  an integer indicating which moment the object should represent (2025, for instance) */
class Moment(private val time: Int):
  ...
  /** Determines the length of time between this moment and another given moment.
    ...
    * 2025, returns 25. The distance is always either positive or zero. */
  ...
  /** Returns the latter of two moments, either this one or the moment provided as a
  ...
  def isIn(interval: Interval) = !interval.start.isLaterThan(this) && !this.isLaterThan(interval.end)
