...
/** Each `Star` object represents a single star on a star map. It records some basic information
  * (an identifier, a location, and a magnitude); in addition, some but not all stars have been
  * assigned a name. (In reality, many stars have multiple alternative names, but this class
  * does not capture that fact.)
  *
...
class Star(val id: Int, val coords: StarCoords, val magnitude: Double, val name: Option[String]):
  ...
  override def toString =
    val nameInString = this.name match {
      case Some(n) => s"$n "
      case None => ""
    }
    s"#${this.id} $nameInString(${this.coords})"
