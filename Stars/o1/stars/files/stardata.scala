...
def parseStarInfo(starAsString: String): Star =
  val traits = starAsString.split(";") 
  val id = traits(3).toInt
  val coords = StarCoords(traits(0).toDouble, traits(1).toDouble)
  val magnitude = traits(4).toDouble
  val name =
    if traits.size == 6 then None
    else Some(traits(6))
  Star(id, coords, magnitude, name)
