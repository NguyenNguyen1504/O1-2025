...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth+2, floorHeight+2):
  ...
  // Note to students: Please do not change the names of the private variables above. There is
  // no need to change the names, and our auto-grader expects the variables to be named like this.


  /** Generates the elements that initially occupy the grid. In the case of a `RobotWorld`  grid,
    * an element is a `Square` object — either a [[Floor]] instance or the [[Wall]] singleton object.
    * Initially, all the edge squares of the robot world are walls and the inner squares are empty
    * `Floor` squares. This method is automatically invoked by the superclass `Grid` to initialize
    * the grid’s contents. (Note to students
    *
  ...
  private def initialSquare(x: Int, y: Int) =
    if x == 0 || y == 0 || x == this.floorWidth + 1 || y == this.floorHeight + 1 then
      Wall
    else
      Floor()
  ...
  /** Creates a new robot into this robot world. The newly created robot body does not yet
    ...
    *
    * @param initialLocation  the initial location of the new robot in this world.
    *                         This method assumes that `location` points to an empty square.
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) =
    ...
    newRobot.locationSquare.addRobot(newRobot)
    newRobot
//    this.robots = this.robots :+ this.robots.head // TODO: This method is implemented quite incorrectly.
//    this.robots.head                              //       Replace these lines with a working implementation.
...
// TODO: implementation missing
  ...
  /** Returns a collection of all the robots in this robot world, in the order they were
    * added to the world. */
  ...
  def robotWithNextTurn: Option[RobotBody] =
    if this.robots.isEmpty || this.turn >= this.robots.size then None
    else Some(this.robots(this.turn))
  ...
  def advanceTurn() =
    robotWithNextTurn.foreach( _.takeTurn() )
    if this.robots.nonEmpty then
      this.turn = (this.turn + 1) % this.robots.size
 // TODO: replace with a proper implementation


  /** Causes all the robots in the world to take a turn, starting with the robot whose turn it is next.
    * (After this is done, the robot who originally was next up, will be that once again.)
    * @see [[robotWithNextTurn]]
    * @see [[advanceTurn]]
    * @see [[RobotBody.takeTurn]] */
  def advanceFullRound() =
    for robotNumber <- this.robots.indices do
      advanceTurn()
      // TODO: replace with a proper implementation
