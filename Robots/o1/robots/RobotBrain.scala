...
trait RobotBrain(initialName: String, val body: RobotBody):
  ...
  /** Returns the coordinates that point at the square that is immediately in front
    * of this robot. */
  def locationInFront = this.location.neighbor(this.body.facing) // TODO: replace with a working implementation

  /** Returns the square that is immediately in front of this robot. */
  def squareInFront = this.body.neighboringSquare(this.body.facing) // TODO: replace with a working implementation
  ...
  def robotInFront: Option[RobotBrain] =
    squareInFront.robot.flatMap( _.brain )
  ...
  def isStuck = !mayMove(East) && !mayMove(South) && !mayMove(West) && !mayMove(North)
  // TODO: replace with an implementation that uses mayMove
  // (Note: it’s fine that the default implementation of mayMove, above, always returns true.
  // That behavior will get overridden in subtypes of RobotBrain, for some types of robots.
  // Here, just take it on faith that mayMove will sometimes return false, too.)
  ...
  /** Moves the robot one square forwards, if there is nothing there. If that square
    ...
    *
    * (Note: This method *only* moves the robot if the square in front of it is
  ...
  def advanceCarefully(): Boolean =
    val advanceSucceeded = squareInFront.isEmpty
    if advanceSucceeded then
