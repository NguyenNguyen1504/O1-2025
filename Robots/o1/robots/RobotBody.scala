...
class RobotBody(val world: RobotWorld, initialLocation: GridPos, initialFacing: CompassDir):
  ...
  var brain: Option[RobotBrain] = None              // most-recent holder (it is possible for a robot to change brains, or it will be, eventually, anyway)
  ...
  /** Returns the coordinates of the robot’s current location within its robot world. */
  ...
  def neighboringSquare(direction: CompassDir) = this.world(this.location.neighbor(direction)) // TODO: replace with a working implementation
  ...
  def spinClockwise() =
    this.facesTowards = this.facesTowards.clockwise
  ...
  /** Relocates the robot within its current world to the square next to the robot’s
    ...
    *
    * This method turns the robot to face in the direction that the robot moves in.
    *
  ...
  def moveTowards(direction: CompassDir) =
    ...
    val moveSucceeded = targetSquare.addRobot(this)
