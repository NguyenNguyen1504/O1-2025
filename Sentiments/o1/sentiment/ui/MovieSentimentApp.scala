...
/** This program loads training data for a [[SentimentAnalyzer]] from a text file
  * that contains movie reviews. It then interacts with the user in the text console,
  * interactively prompting them to enter movie reviews and using the analyzer to
  * report whether those reviews are positive or negative. The session ends when the
  * user enters an empty input (i.e., the empty string or just whitespace).
  *
...
@main def runSentimentApp() =
  ...
  val requestList = LazyList.continually(requestMovieReview())
  requestList.takeWhile( _ != "" ).map(analyze).foreach(println)
