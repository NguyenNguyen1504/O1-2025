...
object chargerView extends View("Charger App"):
  ...
  override def makePic =
    val head =
      if charger.heading == Direction.Right then
        0
      else if charger.heading == Direction.Left then
        180
      else if charger.heading == Direction.Up then
        -90
      else
        90
    background.place((chargerPic).clockwise(head), charger.pos)
