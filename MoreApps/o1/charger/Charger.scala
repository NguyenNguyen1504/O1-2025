...
class Charger(var pos: Pos, val maxSpeed: Double):
  ...
  private var velocity = Velocity(Direction.Right, 0.0) // most-recent holder: the direction the charger most recently turned to
  ...
  def accelerate(dirOfMovement: Direction) =
    if this.heading == dirOfMovement then
      velocity = velocity.faster(1).noFasterThan(this.maxSpeed) // TODO: speed should be adjusted here
    else
      velocity = Velocity(dirOfMovement, 0)  // TODO: speed and angle should be adjusted here
  ...
  def move() =
    this.pos = velocity.nextFrom(this.pos)
end Charger
