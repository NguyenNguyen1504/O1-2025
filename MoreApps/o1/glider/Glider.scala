...
class Glider(var pos: Pos, val maxSpeed: Double):
  ...
  /** Does the following, in order:
    ...
    *    a number of units equal to the glider’saf
  ...
  private def adjustVelocity() =
    if this.isAccelerating then
      this.velocity = this.velocity.faster(1).noFasterThan(this.maxSpeed)
    if this.isTurningLeft then
      this.velocity = this.velocity.changeDirection(this.velocity.direction.counterclockwise(10))
    if this.isTurningRight then
      this.velocity = this.velocity.changeDirection(this.velocity.direction.clockwise(10))
