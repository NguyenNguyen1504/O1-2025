...
class CarEnhancement(consumption: Double, tankSize: Double, fuelInTank: Double, initialPos: Pos):

  import scala.language.adhocExtensions
  object car extends Car(consumption, tankSize, fuelInTank, initialPos):
    ...
    override def location: Pos =        ifImplemented { super.location     } getOrElse initialPos
  ...
  def findRoute(destination: String): IndexedSeq[Segment] =
    this.reachedDestination = false
    val legs = Directions.findRoute(toDirectionsFormat(this.car.location), destination).toIndexedSeq
    val segments = legs.flatMap( _.segments )
