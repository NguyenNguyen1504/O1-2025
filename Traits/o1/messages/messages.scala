...
class Post(content: String, isPublic: Boolean) extends Message(content, isPublic)

class Comment(content: String, original: Post, isPublic: Boolean) extends Message(content, isPublic), Reply(original)
...
@main def testMessages() =
  ...
  println(report(testReply)) // should print: Ave at thee, too! (Re: Ave, Munde!) / not public
