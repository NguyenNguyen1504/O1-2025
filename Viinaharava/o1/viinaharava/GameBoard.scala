...
class GameBoard(width: Int, height: Int, boozeCount: Int) extends Grid[Glass](width, height):
  ...
  def initialElements =
    ...
    allLocations.map(location => Glass(this, location) )
  ...
  private def placeBoozeAtRandom(howMany: Int) =
    Random.shuffle(this.allElements).take(howMany).foreach( _.pourBooze() ) // TODO: implement this method
  ...
  def isOutOfWater: Boolean = this.allElements.filter( !_.isBooze ).forall( _.isEmpty )   // TODO: implement this method

  /** Determines whether the game is over. The game is over if either
    * (a) there are one or more empty booze glasses on the board; or
    * (b) there are no full water glasses on the board. */
  def isGameOver = boozeGlasses.exists( _.isEmpty ) || isOutOfWater   // TODO: fix this implementation
  ...
  def drink(target: Glass): Unit =
    if target.isBooze then
      boozeGlasses.foreach( _.empty() ) // TODO: implement this branch
    else
      target.empty() // TODO: implement this branch
